<template>
  ...
  >
    <template #title>Créer une nouvelle annonce blog</template>
    <template #form>
    ...
    <template #description>
      Créez une annonce pour le blog SolidHive. Elle sera visible par tous les utilisateurs.
    </template>
  </Create>
</template>
...
<script setup lang="ts">
  ...
  const handleAfterCreate = async (createdItem: any) => {
    if (imageFile.value && createdItem?.data?.id) {
      try {
        ...
        toast.success('Annonce créée avec succès avec son image');
      } catch (error) {
        console.error('Error uploading image:', error);
        toast.warning("L'annonce a été créée mais l'image n'a pas pu être uploadée");
      }
    } else {
