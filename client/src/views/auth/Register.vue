  ...
  <div class="flex min-h-screen items-center justify-center bg-gray-100 px-4 py-6">
    <div class="w-full max-w-3xl rounded-lg bg-white p-8 shadow-lg">
      ...
      <p class="mb-6 text-center text-gray-600">
        Rejoignez Solidhive pour accélérer vos projets de développement
      </p>
      ...
      <form class="space-y-5" @submit.prevent="onSubmit">
        <div class="mb-2 grid grid-cols-1 gap-4 md:grid-cols-2">
          ...
          <div>
            <InputForm
              ...
              type="text"
              placeholder="Dupont"
              :error-message="showError('name') ? form.name.$error?.message : ''"
        ...
        <div class="mb-2 grid grid-cols-1 gap-4 md:grid-cols-2">
          <div>
            ...
            >
              <template #hint>
                <div>Utilisez une adresse email professionnelle (pas de Gmail, Yahoo, etc.)</div>
              </template>
            </InputForm>
          </div>
        ...
        <InputForm
          ...
          :error-message="showError('siret') ? form.siret.$error?.message : ''"
          :error-state="showError('siret')"
          @blur="touchedFields.siret = true"
        >
          <template #hint>
            <div>Le numéro SIRET doit comporter 14 chiffres sans espaces ni tirets.</div>
          </template>
        </InputForm>
...
<script setup lang="ts">
  ...
  // Type des champs
  type FormFields = 'name' | 'firstname' | 'email' | 'password' | 'phone' | 'siret';
  ...
  const form = defineForm({
    ...
    email: field(
      ...
      yup
        ...
        .test('is-professional', userErrorMessages.format.businessEmail, (value) => {
          if (!value || !value.includes('@')) return true;
          const domain = value.split('@')[1].toLowerCase();
          return !userErrorMessages.personalEmailDomains.includes(domain);
        })
    ),
    ...
    siret: field(
      ...
      yup
        ...
        .matches(userErrorMessages.patterns.siret, userErrorMessages.format.siret)
    ),
  });
